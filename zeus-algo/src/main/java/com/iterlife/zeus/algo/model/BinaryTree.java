 ...
 *
 * @project zeus
 * @file com.iterlife.zeus.test.TraversingTreeTest.java
 * @version 1.0.0
 * Copyright 2019 - 2019 for Lu Jie
 * https://www.iterlife.com
 *
 **/
package com.iterlife.zeus.algo.model;
...
public class BinaryTree {
	...
	public void noRecPostOrder(TreeNode p) {
		...
		while (p != null) {
			...
			// 当前结点无右子树或右子树已经输出
			while (p != null
					&& (p.rightChild == null || p.rightChild == node)) {
				visted(p);
				// 纪录上一个已输出结点
				node = p;
				if (stack.empty())
	...
	public static void main(String[] args) {
		...
		binTree.nonRecPreOrder(binTree.root);

		System.out.println("***非递归实现****(中序遍历)[DBEACF]遍历*****************");
		binTree.nonRecInOrder(binTree.root);

		System.out.println("***非递归实现****(后序遍历)[DEBFCA]遍历*****************");
		binTree.noRecPostOrder(binTree.root);
	}
}
